...
class ListaDobleH():
		...
		while nodoAux != None:  #Recorre la lista utilizando el nodo auxiliar.
			...
			if self.esMayor(nuevoNodo.getValor(), nodoAux.getValor()) == True:  #Si es mayor el valor a insertar sigue recorriendo.
				...
				if nodoAux == self.inicio:   #Si el valor actual es inicio, se inserta el nuevo valor al principio.
					...
					nuevoNodo.setDerecha(nodoAux)
					nodoAux.setIzquierda(nuevoNodo)
					self.inicio = nuevoNodo
					...
					nuevoNodo.setIzquierda(nodoAux.getIzquierda())
					nodoAux.getIzquierda().setDerecha(nuevoNodo)
					nuevoNodo.setDerecha(nodoAux)
					nodoAux.setIzquierda(nuevoNodo)
					insertado = True
					break
	...
	def imprimirLista(self):
		nodoAux = self.inicio
		while nodoAux != self.fin.getDerecha():
			...
			while nodoAux!=self.fin.getDerecha():
				if nodoAux.getValor() == valor:
					...
					if nodoAux == self.inicio:
						...
						if nodoAux.getDerecha() != None:
							self.inicio = nodoAux.getDerecha()
							nodoAux.getDerecha().setIzquierda(None)
					...
					#Si es el ultimo nodo en la lista
						...
						if nodoAux.getIzquierda() != None:
							self.fin = nodoAux.getIzquierda()
							nodoAux.getIzquierda().setDerecha(None)
							self.tamanio = self.tamanio - 1
							break
					...
					#Si no es ni el primero ni el ultimo
						...
						nodoAux.getIzquierda().setDerecha(nodoAux.getDerecha())
						nodoAux.getDerecha().setIzquierda(nodoAux.getIzquierda())
						self.tamanio=self.tamanio - 1
						break
				else:
					nodoAux = nodoAux.getDerecha()